...
namespace NuminaBit.Services.Ciphers.DES
{
    public class AttackRunner(ICore des): IAttackRunner
    {
        ...
        private static readonly int[] textBitPositionsRound5 = [7, 18, 24, 27, 28, 29, 30, 31];
        private static readonly int[] keyBitPositionsRound5 = [42, 43, 45, 46];
        ...
        public Task<TrialOutcome> RunAlgorithm1On3RoundSingleAsync(ulong key64, int pairs)
        {
            return Task.Run(() =>
            {
                ...
                for (int i = 0; i < pairs; i++)
                {
                    ...
                    int lhs = ExtractBits(PH, textBitPositionsRound3)
                            ^ ExtractBits(CH, textBitPositionsRound3)
                            ^ GetBit(PL, 15)
                            ^ GetBit(CL, 15);
        ...
        public Task<TrialOutcome> RunAlgorithm1On5RoundSingleAsync(ulong key64, int pairs)
        {
            return Task.Run(() =>
            {
                ...
                for (int i = 0; i < pairs; i++)
                {
                    ...
                    // 3-round, no IP/FP
                    ulong cipher = _des.EncryptCustom(plain, ks, rounds: 5, withIP: false, withFP: false);
                    //var b = ToBinaryString(cipher, 64);
                    ...
                    int lhs = ExtractBits(PL, textBitPositionsRound5)
                            ^ ExtractBits(CL, textBitPositionsRound5)
                            ^ GetBit(PH, 15)
                            ^ GetBit(CH, 15);
